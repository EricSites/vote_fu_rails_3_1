    ...
    module Voteable #:nodoc:
      ...
      module SingletonMethods
        ...
        def tally(*args)
          ...
          options_for_tally(conditions, options).order("count_all DESC")
        end
        ...
        def options_for_tally(conditions, options = {})
            options.assert_valid_keys :start_at, :end_at, :conditions, :at_least, :at_most, :order, :limit unless options.blank?
            ...
            select = "#{table_name}.*, COUNT(#{Vote.table_name}.id) AS count_all"
            ...
            options_for_tally = select(select).where(conditions).where(options[:where]).joins(joins).group(group_by).having("count_all > 0")
